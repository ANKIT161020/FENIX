...
# The keys we expect in the final JSON
EXPECTED_JSON_KEYS = [
    "Recipient_Name",
    "Certificate_ID",
    "Verification_Link",
...
def extract_text_easyocr(image_path):
    ...
    try:
        ...
        print(f"[ERROR] Error with EasyOCR on {image_path}: {e}", file=sys.stderr)
        return ""
...
def extract_text_tesseract(image_path):
    ...
    try:
        ...
        print(f"[ERROR] Error with Tesseract on {image_path}: {e}", file=sys.stderr)
        return ""
...
def send_to_deepseek(extracted_text):
    ...
    This version extracts the JSON array from the response even if extra commentary is present.
    """
...
Return the extracted details as valid JSON but do not include your thinking or markdown.
The JSON should be an array of objects, each with the following keys exactly:
...
def main():
    ...
    # 2. For each certificate, do OCR + send to DeepSeek
    all_cert_data = []
    for path in file_paths:
        ...
        print(f"[INFO] Extracted text for {path} (length {len(extracted_text)}): {extracted_text}", file=sys.stderr)
        deepseek_result = send_to_deepseek(extracted_text)

        # deepseek_result is expected to be an array of objects
        for item in deepseek_result:
            item["File_Path"] = path
            all_cert_data.append(item)
    ...
    import uuid
    csv_filename = "certificates_extracted_" + uuid.uuid4().hex + ".csv"
